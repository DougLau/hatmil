...
impl<'c> From<&'c str> for Value<'c> {
    fn from(v: &'c str) -> Self {
        Value { iter: CharIter::Borrowed(v) }
    }
}

impl From<String> for Value<'_> {
    fn from(v: String) -> Self {
        Value { iter: CharIter::Owned(v) }
    }
}
...
impl<'c> From<Cow<'c, str>> for Value<'c> {
    fn from(v: Cow<'c, str>) -> Self {
        match v {
            Cow::Borrowed(v) => Value { iter: CharIter::Borrowed(v) },
            Cow::Owned(v) => Value { iter: CharIter::Owned(v) },
        }
    }
}
...
impl From<char> for Value<'_> {
    fn from(v: char) -> Self {
        Value { iter: CharIter::Owned(v.to_string()) }
    }
}
